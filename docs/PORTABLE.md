...
4. `Connect.Desktop.exe` を起動し、画像込みモデル（.srig）を開きます。カメラ開始は利用者の操作で行います。
...
The default model extension is `.srig` (JSON content); existing embedded-image `.json` files remain supported.
